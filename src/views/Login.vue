<template>
  <div class="h-full w-full">
        <div class="bg-blue-900 py-3 px-3 text-white fixed w-full bottom-0 flex justify-center">
          <h3 class="text-xl">{{ isRegister ? '注册' : '登录' }}</h3>
        </div>
         <div class="bg-blue-500 fixed top-0 w-full py-2 px-3 text-white flex justify-between">
            <p>欢迎来到我们自己的开发项目</p>
            <p
...
export default {
    ...
    methods: {
        ...
        async submit() {
            ...
            const data = await this.$store.dispatch('login',{account, password})
            if (data) {
                this.$showModal({
                    context: data
                })
        ...
        async registerHandler() {
            ...
            this.$catch(async () => {
                const { data: { message, success } } = await register({ name, account, password, date })
                if (success) {
                    this.switchRegister()
                    this.$showModal({
                        title: '提示',
...
.login{
    background: url('http://localhost:8080//static/login.png') 100% 100% no-repeat;
}
